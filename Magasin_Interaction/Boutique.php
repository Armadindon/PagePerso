...
if(isset($_POST["idP"])){
  $idC = $dbh ->query("SELECT idMagasin FROM Magasin WHERE login=\"".$_SESSION["login"]."\";")->fetch()[0];
  ...
  $qty = $_POST["Qty"];
  $idP=$_POST['idP'];
  $req = $dbh -> query("INSERT INTO DetailPanierM VALUES ($idC,$idP,$qty);");
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <div class="Contenue">
    <h1>Quelle catégorie de produit voulez-vous consulter ?</h1>
    <form method="post">
      ...
      <table>
        <table class="table table-striped">
          ...
          <tbody>
            <?php
            $idC = $dbh ->query("SELECT idMagasin FROM Magasin WHERE login=\"".$_SESSION["login"]."\";")->fetch()[0];
            if ($cat == "jeux-video" || $_POST["Categorie"] == "consoles") {
              $req = $dbh ->query("SELECT Nom,Editeur,idProd FROM Produit NATURAL JOIN Categorie WHERE idProd NOT IN(SELECT idProd FROM DetailPanierM WHERE idMagasin=$idC) AND Categorie.libCat =\"".$cat."\" ;");
            }else{
              $req = $dbh ->query("SELECT Nom,idProd FROM Produit NATURAL JOIN Categorie WHERE idProd NOT IN(SELECT idProd FROM DetailPanierM WHERE idMagasin=$idC) AND Categorie.libCat =\"".$cat."\" ;");
            }
            while ($ligne=$req->fetch()) {
              if ($cat == "jeux-video" || $cat == "consoles") {
